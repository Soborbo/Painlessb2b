...
export const POST: APIRoute = async ({ request }) => {
  ...
  for (const p of prospects) {
    try {
      ...
      // Duplicate check: same name + postcode (also check name-only if no postcode)
      if (p.postcode) {
  ...
  return new Response(JSON.stringify({ imported, skipped, errored, total: prospects.length }), {
    headers: { 'Content-Type': 'application/json' },
  });
};
